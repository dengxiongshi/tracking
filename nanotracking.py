...
from nanodet.inference import *
print('Note: running nanodet detector')
...
def main(opts):
    ...
    while True:
        ...
        nano_result = predictor.inference_from_tensor(img0)     
        nano_result = nano_result[nano_result[:, 4] > 0.35]
        string_ += 'nanodet-time: ' + str(round(time.time() - start_time, 3)) 
        ...
        current_tracks = tracker.update(nano_result, img0)  # List[class(STracks)]      
...
def plot_img(img, frame_id, results, save_dir):
    ...
    for tlwh, id, cls in zip(tlwhs, ids, clses):
        ...
        # draw a rect
        cv2.rectangle(img_, tlbr[:2], tlbr[2:], get_color(id), thickness=2, )
        # note the id and cls
        text = f'id: {id}'
...
def parse_opt():
    ...
    parser.add_argument('--obj', type=str, default=r'../test.mp4', help='video NAME or images FOLDER NAME')
