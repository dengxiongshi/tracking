...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if not (frame_id + 1) % detect_per_frame:
            ...
            string_ += 'preprocess time: ' + str(round(t2 - t1, 3)) + 'ms  '
            # Inference
            visualize = increment_path(save_dir / Path(path).stem, mkdir=True) if visualize else False
            pred = model(im, augment=augment, visualize=visualize)
            t3 = time_sync()
            # dt[1] += t3 - t2
            string_ += 'inference time: ' + str(round(t3 - t2, 3)) + 'ms  '
            # NMS
            pred = non_max_suppression(pred, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det)
            # dt[2] += time_sync() - t3

            string_ += 'NMS time: ' + str(round(time_sync() - t3, 3)) + 'ms  '
            ...
            # det = pred
                ...
                t4 = time_sync()
                # Run SORT
                current_tracks = track.update(det, im0)
                t5 = time_sync()
                string_ += 'tracker time: ' + str(round(t5 - t4, 3)) + 'ms  '
        else:
            string_ = f'----------track frame {frame_id}----------'
            im0 = im0s.copy()
            t1 = time_sync()
            current_tracks = track.update_without_detection(None, im0)
            t8 = time_sync()
            string_ += 'tracker time: ' + str(round(t8 - t1, 3)) + 'ms  '
        ...
        string_ += 'total time: ' + str(round(t6 - t1, 3)) + 'ms'
        print(string_)
...
def parse_opt():
    ...
    parser.add_argument('--detect_per_frame', type=int, default=2, help='choose how many frames per detect')
